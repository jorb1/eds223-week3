...
nz_height3 <- nz_height[nz_height_logical, ]
```
...
```{r}
nz_height_left_join <- st_join(nz_height, nz, join = st_intersects) |>
...
```{r}
cycle_hire_join <- st_join(cycle, cycle_hire_osm,
...
tm_shape(nz) +
  ...
  tm_symbols(col = "coral", alpha = 0.5) +
  tm_shape(nz_pos) +
  tm_symbols(col = "seagreen", alpha = 0.5)
